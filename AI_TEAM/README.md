...
當 `queue.json` 沒有 pending / running task 時，planner 會先讀 backlog、current task、product readiness、launch checklist、fix roadmap、QA runtime report，再依產品主線補入下一個安全任務，不會只因 queue 空掉就停住。預設補題順序是 Inbox、Channels、IG metadata、Contacts、Automations、Analytics、Billing / PayUNI Sandbox、Launch readiness sweep。

補題不是一次性清單。若第一輪產品主線都跑過，runner 會產生 `cycle-2`、`cycle-3` 類型的下一輪產品任務，直到 readiness 文件明確標示已可上線，或遇到人工 hard stop。

如果 `qa` 或 `browser-qa` 失敗，runner 會自動新增 fix task，下一輪優先修測試或 Browser QA 失敗，不會只停在報告。
...
- queue 空時一樣會自動補題
- QA / Browser QA fail 時一樣會自動產生 fix task
