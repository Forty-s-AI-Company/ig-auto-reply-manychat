...
export function MockTesterClient() {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div>
        <h2 className="text-2xl font-semibold">Mock Tester</h2>
        <p className="text-sm text-zinc-400">本機模擬 inbound webhook，會建立 contact、conversation、message 並觸發 automation。</p>
      </div>
      <section className="space-y-3 rounded-lg border border-zinc-800 bg-zinc-900 p-4">
        <label className="block text-sm">
          <span className="text-zinc-300">External ID</span>
        ...
        <button onClick={send} className="rounded-md bg-cyan-500 px-4 py-2 text-zinc-950">送出 Mock Inbound</button>
      </section>
