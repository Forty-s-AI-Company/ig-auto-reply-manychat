      ...
      {isConfirmOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="presentation">
          <div
            ...
            data-testid="admin-invoice-refund-dialog"
            className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-lg border border-red-200 bg-white p-5 shadow-xl"
          >
            <h2 id={`refund-invoice-title-${invoiceId}`} className="text-base font-semibold text-[#111827]">
              確認標記退款？
            </h2>
            <p className="mt-2 text-sm leading-6 text-[#475467]">
              這會把帳單標記為已退款，並執行推薦折抵取消 / 沖回。此動作只處理 InboxPilot 內部狀態，不會自動向 PayUNI 發起退款，也不會退回使用者信用卡款項。
            </p>
