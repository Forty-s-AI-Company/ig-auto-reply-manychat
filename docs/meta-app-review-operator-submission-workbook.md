...
| Successful OAuth return + connected channel | 已可直接錄 | A real reviewer-safe staging connect now succeeds and the connected channel is visible in Channels plus the sidebar dropdown. | Keep the reviewer-safe Instagram asset/session and complete final redaction. |
| Inbox message read / reply | 已可直接錄 | The reviewer-safe staging tenant now shows a visible synthetic conversation in Inbox. | Record it from staging after final redaction and secure credential handoff. |
| Contacts workspace scope | 已可直接錄 | The reviewer-safe staging tenant now shows a visible synthetic contact under the same workspace/channel scope. | Record it from staging after final redaction and secure credential handoff. |
...
- Reviewer-safe user and workspace are ready, and the reviewer-safe staging evidence lane remains redaction-safe.
