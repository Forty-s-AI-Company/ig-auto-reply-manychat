...
Last updated: 2026-07-03 (staging tenant + automation draft verified).
...
- `[ ]` Inbox and Contacts show reviewer-safe seeded content.
...
- `[ ]` Reviewer-safe Instagram asset is available.
- `[ ]` Asset can complete the staging OAuth flow.
- `[ ]` Asset name/handle/profile are safe for screenshots or recording.
...
- `[ ]` Channels / Connect flow is safe to record.
- `[ ]` Inbox shows reviewer-safe conversation only.
- `[ ]` Contacts shows reviewer-safe contact only.
- `[x]` Automations shows reviewer-safe draft only.
- `[ ]` If connected-channel proof is required, OAuth completes safely.
...
- reviewer-safe Instagram asset preparation
- reviewer-safe Facebook Page / Business linkage if required
- reviewer-safe staging Inbox / Contacts synthetic data lane
