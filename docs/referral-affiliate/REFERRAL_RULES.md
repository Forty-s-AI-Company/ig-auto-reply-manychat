...
- 推薦折抵只能折抵方案費，不可提現、不可轉讓
- 折抵金先進 `pending`，超過 7 天退款觀察期後才轉成 `available`
- `available` 後 30 天內要使用，逾期失效
- 單筆 invoice 最低可折到 0 元
- 退款觀察期內退款會取消 pending 折抵；已使用後才退款時，建立 clawback / debit 沖回紀錄
