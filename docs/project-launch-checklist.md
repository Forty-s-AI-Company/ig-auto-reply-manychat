...
- [x] Coverage 達 80% 以上。
- [x] 1000 user load test 已執行並產出瓶頸、快取、rate limit 方案。2026-05-31 local Next dev + remote Supabase 測試：1000 virtual users / 1001 requests / 673 errors or timeouts / 67.23% error rate / p95 幾乎打滿 30s timeout，判定目前不能承受 1000 人同時在線。詳細結果見 `docs/performance-load-test-results-2026-05-31.md`；正式上線前仍需 staging/production-like URL 重跑。
- [x] 1000 user load test 第二波優化已完成並記錄。2026-06-01 已完成 Dashboard / Analytics / AdminShell / Inbox / Contacts API / Conversations API / Broadcasts page 短 TTL cache 與 payload 精簡，並修正 load test 成功判斷。Production-mode local smoke：20 users / 58 requests / 0 errors / 0% error rate。Production-mode 1000 single-wave：1106 requests / 554 errors or timeouts / 50.09% error rate / p95 約 30s，仍不可宣稱可承受 1000 人同時在線；剩餘 blocker 是 Next request runtime + DB pool 在瞬間高併發下排隊，需導入 dedicated queue worker / durable queue / materialized summaries。
- [x] Dedicated queue worker foundation 已建立。2026-06-01 已新增 BullMQ / Redis queue adapter；job 仍先寫入 PostgreSQL `Job` table 作為 source of truth，設定 `REDIS_URL` 後會 mirror 到 BullMQ，由 `npm run worker` 的 dedicated worker 消費；未設定 Redis 時保留 DB polling fallback。正式部署仍需提供 Redis instance 並在 staging 重跑 1000-user load test。
