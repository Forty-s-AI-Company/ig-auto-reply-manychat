...
- `[x]` `npm run test:e2e:inbox` now runs `e2e:admin:ensure` before the authenticated Inbox smoke.
- `[x]` The prior Inbox smoke failure was narrowed to missing local E2E admin seed, not a product route failure.

Remaining:

- `[ ]` Continue checking other authenticated Playwright scripts for missing TEST_DATABASE_URL seed preflight.
- `[ ]` Keep all E2E seed helpers away from production DB.
...
- 已完成：Channels 設定側欄 anchor、Instagram 權限外連、方案頁與 AI 設定連結補齊 keyboard-visible focus treatment。
...
- 已完成：Broadcasts 刪除操作改為 in-app confirmation dialog，不再直接執行 DELETE。
...
- 已完成：手機版主選單的開啟、關閉與 nav links 補齊 keyboard-visible focus treatment。
- 已完成：Mobile nav decorative icons 補 `aria-hidden`，維持既有 IA 與 release-mode nav filtering 不變。
- 下一步：若沒有人工阻塞，繼續掃完整 onboarding path 的 browser smoke，或回到 Billing / Referrals / Admin financial surfaces 做剩餘 mobile polish。

## 2026-07-03 - OAuth fallback control focus polish

- 已完成：Telegram token fallback form 補 input/button focus treatment 與 inline error status。
