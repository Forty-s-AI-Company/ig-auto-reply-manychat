...
## Latest - 2026-06-30 AI_TEAM single-worker replay

Current status:

- `[x]` Added `--only-worker=<name>` / `AI_TEAM_ONLY_WORKER` so the runner can replay a single worker using the existing loop state.
- `[x]` Documented the single-worker replay path in `AI_TEAM/README.md` and `AI_TEAM/RUNNER_DESIGN.md`.
- `[x]` Kept the change scoped to runner infrastructure only.

Remaining:

- `[ ]` Exercise `--only-worker=git-delivery` in a real disposable branch with dry-run disabled.
- `[ ]` Decide whether reporter/browser-qa should gain the same replay examples in README snippets.
